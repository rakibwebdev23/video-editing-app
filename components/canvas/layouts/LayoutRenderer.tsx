...
export default function LayoutRenderer({ layout, canvasWidth, canvasHeight, usedZones = [], children, onDrop }: LayoutRendererProps) {
  const zones = getZones(layout, canvasWidth, canvasHeight);

  return (
    <>
      {zones.map((zone, idx) => {
        const isUsed = usedZones.includes(idx);
        ...
        return (
          <div
            ...
            style={{
              ...
              width: zone.width,
              height: zone.height,
              border: isUsed ? 'none' : '1px dashed rgba(59,130,246,0.3)',
